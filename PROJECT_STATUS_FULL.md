...
Для реального запуска проект пока не готов без доработок: 1С-интеграция умеет отправлять события HTTP-клиентом, но еще без фонового воркера и retry/backoff; платежи ручные, production security зависит от реальных env-настроек, нет реального деплоя, а часть демо-медиа подключена неполностью.
...
|---|---|---|---|
| 1С | Частично / очередь + HTTP-отправка | `integrations/models.py::SyncEvent`, `integrations/services.py::queue_booking_sync`, `send_sync_event`, `send_onec_sync`, `.env.example` `ONEC_API_URL`, `ONEC_API_TOKEN` | Есть отправка pending/failed событий и Bearer token; нет фонового воркера, retry/backoff и обработки входящих событий |
...
| VK сообщения | Частично | `send_vk_message`, `VKNotificationsToggleView`, `VKTestNotificationView` | Нужен `VK_GROUP_TOKEN`, разрешение сообщений от пользователя; JSON-ошибки VK разбираются |
...
- `python manage.py test` — 50 тестов, все OK.
- `python manage.py check --deploy` — 0 issues при `DEBUG=False`, сильном `SECRET_KEY` и включенных HTTPS/HSTS-настройках.

Security-настройки вынесены в env:
...
- Создание, подтверждение, выдача, возврат, продление, отмена и no-show обернуты в транзакции с `select_for_update`; для высокой нагрузки все равно стоит добавить ограничения/идемпотентность на уровне БД и сервиса.
...
- `ONEC_API_TOKEN` используется HTTP-отправителем 1С как Bearer token.
...
1. 1С-интеграция требует production-доводки.
   - Файлы: `integrations/services.py`, `integrations/models.py`.
   - Создается `SyncEvent`, есть HTTP-клиент отправки payload в `ONEC_API_URL` и статусы `sent/failed`; нет фонового воркера с retry/backoff и входящих endpoint'ов.

2. Production security параметризована, но требует реальных env-значений.
   - Файл: `velorent/settings.py`.
   - Подтверждение: `python manage.py check --deploy` проходит без issues при production env-настройках.
...
4. Docker-конфигурация dev-уровня.
   ...
   - Используется `runserver`; нет gunicorn/uvicorn, collectstatic и nginx.
   - `docker-compose.yml` поднимает Postgres и передает Django PostgreSQL-настройки; добавлен healthcheck БД.

5. VK-отправка требует боевой проверки.
   - Файл: `integrations/vk_notifications.py::send_vk_message`.
   - JSON-ответ VK разбирается; остаются реальные токены, разрешения сообщений и эксплуатационные ретраи.
...
- `migrate --check` — OK.
- `test` — 50 tests OK.
- `check --deploy` — 0 issues при production env-настройках.
...
4. 1С:
   - вынести `send_onec_sync` в фоновой воркер/планировщик;
   - retry/backoff;
...
Для реального клиента оценка существенно ниже: 1С-синхронизации нужен фоновой воркер и retry/backoff, нет онлайн-оплаты, production security нужно включать реальными env-значениями, нет надежного деплоя и полноценной эксплуатационной инфраструктуры. Это нормальная стадия для дипломного MVP, но не для коммерческой эксплуатации без следующего этапа разработки.
