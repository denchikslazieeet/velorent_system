...
## 8. Настроить доверенный HTTPS-сертификат

В ISPmanager откройте `SSL-сертификаты` и выпустите бесплатный сертификат
Let's Encrypt для:

- `velo-rent-chita.ru`;
- `www.velo-rent-chita.ru`.

Не выбирайте самоподписанный сертификат: браузеры считают его недоверенным и
показывают предупреждение `Не защищено`.

После выпуска откройте `Сайты`, выберите `velo-rent-chita.ru`, нажмите
`Изменить`, включите защищённое соединение и назначьте созданный сертификат
Let's Encrypt. Для сайта должен быть включён автоматический переход с HTTP на
HTTPS.

Проверка с компьютера или из Shell-клиента:

```bash
curl -I https://velo-rent-chita.ru/
```

Команда должна вернуть HTTP-ответ без ошибки проверки сертификата.
...
## 10. Настроить уведомления по расписанию
...
Первая задача закрывает прошедшие невыданные брони, освобождает велосипеды,
отправляет одноразовые напоминания клиентам перед выдачей и возвратом, а также
предупреждает операторов о просрочке. Вторая повторяет отправку писем после
временных ошибок SMTP.

## 11. Настроить второй домен
